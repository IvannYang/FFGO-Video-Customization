...
def build_wan22_pipeline(
    ...
    lora_low="/workspace/Models/10_LargeMixedDatset_wan_14bLow_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4/checkpoint-600.safetensors",
    lora_high="/workspace/Models/10_LargeMixedDatset_wan_14bHigh_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4/checkpoint-600.safetensors",
...
def infer_video(
    ...
    *,
    ...
    idx = len(os.listdir(save_path)) + 1
    pref = f"{idx:08d}"

    if video_length == 1:
        img = sample[0, :, 0].permute(1, 2, 0).cpu().numpy() * 255
        Image.fromarray(img.astype(np.uint8)).save(os.path.join(save_path, pref + ".png"))
        return os.path.join(save_path, pref + ".png")
    else:
        out_mp4 = os.path.join(save_path, pref + ".mp4")
...
if __name__ == "__main__":
    ...
    # json_path = "/workspace/Results/06_mixed/fix4.json"
    import pandas as pd
    df = pd.read_csv('/workspace/Data/01_zongxia_test_checkpoint10/checkpoint10-v19.csv')
    # Print the number of entries
    print(f"Loaded {len(df)} entries")
    paths = [f'/workspace/Data/01_zongxia_test_checkpoint10/combined_{i}.png' for i in list(df['video_num'])]
    captions = ["ad23r2 the camera view suddenly changes. " + ele for ele in list(df['caption'])]
    ...
    pipe, vae, boundary, device = build_wan22_pipeline(
        ...
        model_name="/workspace/hfhome/hub/models--Wan-AI--Wan2.2-I2V-A14B/snapshots/206a9ee1b7bfaaf8f7e4d81335650533490646a3",
                #     lora_low="/workspace/Models/11_MixedDatset_wan_14bLow_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4/checkpoint-500.safetensors",
                # lora_high="/workspace/Models/11_MixedDatset_wan_14bHigh_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4/checkpoint-500.safetensors",
lora_low="/workspace/Models/10_LargeMixedDatset_wan_14bLow_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4/checkpoint-600.safetensors",
        lora_high="/workspace/Models/10_LargeMixedDatset_wan_14bHigh_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4/checkpoint-600.safetensors",
    )
        # Example: iterate and print each entry
    for i in range(len(paths)):
        ...
        video_path = infer_video(
            ...
            video_length=81,
            validation_image_start=file_path,
            prompt=text,
            # save_path="/workspace/Results/06_mixed/2"
            # save_path = "/workspace/zongxia_result/03_real_tests"
            save_path = "/workspace/zongxia_result/checkpoint10"
            # save_path = "/workspace/zongxia_result/checkpoint10-v11"
        )
