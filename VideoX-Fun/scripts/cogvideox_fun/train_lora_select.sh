...
accelerate launch --mixed_precision="bf16" scripts/cogvideox_fun/train_lora_select.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --train_data_dir=$DATASET_NAME \
  --train_data_meta=$DATASET_META_NAME \
  --image_sample_size=1024 \
  ...
  --learning_rate=1e-04 \
  --seed=42 \
  --output_dir="/fs/nexus-projects/DroneHuman/jxchen/data/06_VideoRAG/Results/03_test_cogvideox_select_run3" \
  --gradient_checkpointing \
  --mixed_precision="bf16" \
  --adam_weight_decay=3e-2 \
  --adam_epsilon=1e-10 \
