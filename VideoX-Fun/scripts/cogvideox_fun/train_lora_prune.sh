...
export DATASET_NAME="/fs/nexus-projects/DroneHuman/jxchen/data/06_VideoRAG/Data/Minimalism"
export DATASET_META_NAME="/fs/nexus-projects/DroneHuman/jxchen/data/06_VideoRAG/Data/Minimalism/metadata.json"
# export NCCL_IB_DISABLE=1
# export NCCL_P2P_DISABLE=1
NCCL_DEBUG=INFO

accelerate launch --mixed_precision="bf16" scripts/cogvideox_fun/train_lora_prune.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --train_data_dir=$DATASET_NAME \
  --train_data_meta=$DATASET_META_NAME \
  --image_sample_size=1024 \
  ...
  --learning_rate=1e-04 \
  --seed=42 \
  --output_dir="/fs/nexus-projects/DroneHuman/jxchen/data/06_VideoRAG/Results/02_test_cogvideox_video_002" \
  --gradient_checkpointing \
  --mixed_precision="bf16" \
  --adam_weight_decay=3e-2 \
  --adam_epsilon=1e-10 \
  ...
  --train_mode="normal" \
  --saved_layers_txt="/fs/nexus-projects/DroneHuman/jxchen/data/06_VideoRAG/Results/02_test_cogvideox_prune_002/changed_layers.txt"
  # --train_mode="inpaint" 