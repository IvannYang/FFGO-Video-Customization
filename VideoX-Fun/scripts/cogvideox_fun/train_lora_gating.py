...
from videox_fun.utils.lora_utils_ours import create_network, merge_lora, unmerge_lora, lora_gate_sparsity_loss
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                '''
                add lora gate sparsity loss
                '''
                lora_gate_loss= lora_gate_sparsity_loss(gates, lam=2e-4, use_sigmoid=True)
                
                loss = F.mse_loss(noise_pred.float(), target.float(), reduction="mean")

                loss= loss + lora_gate_loss
