...
def log_validation(vae, text_encoder, tokenizer, transformer3d, network, args, accelerator, weight_dtype, global_step, h, w):
    try:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("If you want to resume training from a lora weight, input its path."),
    )
    ...
    #     type=str,
    #     nargs="+",
    #     default=["to_k", "to_v", "to_q", "to_out", "ff.net.0", "ff.net.2"],
...
def double_batch_image_video_repeat(videos, texts, start_first_frame=False):

    bsz= videos.shape[0]
    videos= torch.cat([videos, videos], dim=0)
    texts = texts * 2
    ...
    for i in range(bsz):
        ...
        # video repeat a random frame
        video = videos[i]
        if start_first_frame:
            frame_idx = 0
        else:
        ...
        video_selected = video[frame_idx:frame_idx+1, :, :, :]

        # print("video_selected shape is ", video_selected.shape)

        video_repeat = video_selected.repeat(videos.shape[1], 1 ,1, 1)

        # print("video_repeat shape is ", video_repeat.shape)

        # exit(0)


        videos[j] = video_repeat
        ...
        motion_segment= text_segments[1]
        motion_segment= ' '
        text = text_segments[0] + ',' + motion_segment + ',' + text_segments[2]
...
def main():
    ...
    if args.lora_path is not None:
        ...
        pipe= merge_lora(pipeline, lora_path, lora_weight, device='cuda')
        print(f"load lora from {lora_path} with weight {lora_weight}.")

        vae = pipe.vae
        text_encoder = pipe.text_encoder
        transformer3d = pipe.transformer
    ...
    # print('sample_n_frames_bucket_interval', sample_n_frames_bucket_interval)
    ...
    print("image learning rate is {}".format(IMAGE_LEARNING_RATE))
    ...
    if args.enable_bucket:
        ...
        # print("Using bucket sampler.")
        # print(f"aspect_ratio_sample_size: {aspect_ratio_sample_size}")
        # exit(0)
        ...
        def collate_fn(examples):
            ...
            if data_type == 'image':
                ...
                # print('random_downsample_ratio', random_downsample_ratio)
                # '''
                # random_downsample_ratio 1.25
                # '''
                ...
                # print('aspect_ratio_sample_size', aspect_ratio_sample_size)
                ...
                # print('aspect_ratio_random_crop_sample_size', aspect_ratio_random_crop_sample_size)
                
                batch_video_length = args.video_sample_n_frames + sample_n_frames_bucket_interval


                # print('batch_video_length', batch_video_length)
                # print('args.video_sample_n_frames', args.video_sample_n_frames)
                # print('sample_n_frames_bucket_interval', sample_n_frames_bucket_interval)
                # exit(0)
            ...
            # print('h w', h, w)
            # print(f"closest_size: {closest_size}, closest_ratio: {closest_ratio}, random_downsample_ratio: {random_downsample_ratio}")
            # exit(0)
            ...
            # print(f"closest_size: {closest_size}, closest_ratio: {closest_ratio}, random_downsample_ratio: {random_downsample_ratio}")
            # '''
            # closest_size: [np.float64(921.6), np.float64(716.8)], closest_ratio: 1.29, random_downsample_ratio: 1.25                                                                                       
            # closest_size: [912, 704], closest_ratio: 1.29, random_downsample_ratio: 1.25     
            # '''
            # exit(0)
            ...
            for example in examples:
                ...
                # '''
                # resize_size (938, 704)
                # closest_size [912, 704]
                # pixel_values.shape torch.Size([1, 3, 832, 624])
                # '''

                '''
                resize_size (341, 192)
                closest_size [336, 192]
                pixel_values.shape torch.Size([49, 3, 640, 360])
                '''
                # exit(0)
                ...
                # print('pixel_values.min(), pixel_values.max()', new_examples["pixel_values"][-1].min(), new_examples["pixel_values"][-1].max())
                # print('pixel_values.shape', new_examples["pixel_values"][-1].shape)
                # '''
                # pixel_values.min(), pixel_values.max() tensor(-1.) tensor(1.)
                # pixel_values.shape torch.Size([1, 3, 912, 704])
                # '''
                # exit(0)
                ...
                # print('local_latent_length', local_latent_length)
                # print('local_video_length', local_video_length)
                # print('additional_frames', additional_frames)
                # print('batch_video_length', batch_video_length)
                # '''
                # local_latent_length 1
                # local_video_length 1
                # additional_frames 0
                # batch_video_length 1
                # '''
                # exit(0)
            ...
            # print('enable_text_encoder_in_dataloader', args.enable_text_encoder_in_dataloader)
            # print('pixel_values.shape', new_examples["pixel_values"].shape)
            # '''
            # enable_text_encoder_in_dataloader False
            # pixel_values.shape torch.Size([1, 1, 3, 912, 704])
            # '''
            # exit(0)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            pixel_values, texts= double_batch_image_video_repeat(pixel_values, texts)
            ...
            if epoch == first_epoch and step == 0:
                ...
                # print('pixel_values.shape', pixel_values.shape)
                # '''
                # pixel_values.shape torch.Size([1, 3, 1, 912, 704])
                # '''
                # exit(0)
                ...
                for idx, (pixel_value, text) in enumerate(zip(pixel_values, texts)):
                    ...
                    # print("prompt:", text)
                    # print("pixel_value shape:", pixel_value.shape)
                    # exit(0)
                if args.train_mode != "normal":
            ...
            with accelerator.accumulate(transformer3d):
                ...
                # print('pixel_values', pixel_values.shape, pixel_values.dtype, pixel_values.min().item(), pixel_values.max().item())
                # '''
                # pixel_values torch.Size([1, 1, 3, 912, 704]) torch.bfloat16 -1.0 1.0
                # '''
                # exit(0)
                ...
                with torch.no_grad():
                    ...
                    def _batch_encode_vae(pixel_values):
                        ...
                        for i in range(0, pixel_values.shape[0], bs):
                            ...
                            # print('pixel_values_bs', pixel_values_bs.shape)
                            # '''
                            # pixel_values_bs torch.Size([1, 3, 1, 912, 704])
                            # '''
                            ...
                            # print('pixel_values_bs', pixel_values_bs.shape)
                            # '''
                            # pixel_values_bs torch.Size([1, 16, 1, 114, 88])
                            # '''
                            # exit(0)
                    ...
                    # print('latents', latents.shape, latents.dtype, latents.min().item(), latents.max().item())
                    # '''
                    # latents torch.Size([1, 16, 1, 114, 88]) torch.bfloat16 -2.421875 2.203125
                    # '''
                    # exit(0)
                ...
                # print('args.train_sampling_steps', args.train_sampling_steps)
                # '''
                # args.train_sampling_steps 1000
                # '''
                # print('timesteps', timesteps)
                # print('noise', noise.shape, noise.dtype, noise.min().item(), noise.max().item())
                # '''
                # timesteps tensor([303], device='cuda:0')
                # noise torch.Size([1, 1, 16, 114, 88]) torch.bfloat16 -4.40625 4.25
                # '''
                # exit(0)
                ...
                def _prepare_rotary_positional_embeddings(
                    ...
                    # print('sample_height, sample_width', unwrap_model(transformer3d).config.sample_height, unwrap_model(transformer3d).config.sample_width)
                    # '''
                    # sample_height, sample_width 60 90
                    # '''
                    # exit(0)


                    # print('grid_height, grid_width', grid_height, grid_width)
                    # print('base_size_height, base_size_width', base_size_height, base_size_width)
                    # '''
                    # height, width 912 704
                    # grid_height, grid_width 57 44
                    # base_size_height, base_size_width 30 45
                    # '''
                    # exit(0)
                ...
                # print('image_rotary_emb', image_rotary_emb[0].shape, image_rotary_emb[1].shape)
                # '''
                # prompt_embeds torch.Size([1, 226, 4096]) torch.bfloat16
                # image_rotary_emb torch.Size([2508, 64]) torch.Size([2508, 64])
                # '''
                ...
                print('Loss shape:', loss.shape, 'loss:', loss.item())
                exit(0)
