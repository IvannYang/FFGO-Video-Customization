...
export DATASET_NAME="/workspace/Data/06_mixed_data_Large"
export DATASET_META_NAME="/workspace/Data/06_mixed_data_Large/train_data_mixed_final.json"
# NCCL_IB_DISABLE=1 and NCCL_P2P_DISABLE=1 are used in multi nodes without RDMA. 
# export NCCL_IB_DISABLE=1
# export NCCL_P2P_DISABLE=1
NCCL_DEBUG=INFO

accelerate launch --num_processes=2 --multi_gpu --mixed_precision="bf16" scripts/wan2.2/train_lora_transition_14b.py \
  ...
  --learning_rate=1e-04 \
  --seed=42 \
  --output_dir="/workspace/Models/11_MixedDatset_wan_14bLow_f81_LongCaption_StartMatch_run_r128_a128_3obj_Longrun_B4" \
  --mixed_precision="bf16" \
  --adam_weight_decay=3e-2 \
  --adam_epsilon=1e-10 \
  --vae_mini_batch=1 \
