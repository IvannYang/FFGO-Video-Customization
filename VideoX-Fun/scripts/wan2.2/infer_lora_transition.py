...
def log_validation(vae, text_encoder, tokenizer, transformer3d, lora_path, config, args, accelerator, weight_dtype, global_step, h, w):
    try:
        ...
        print(f"Load lora from {lora_path} for validation")

        pipeline = merge_lora(pipeline, lora_path, 1, device=accelerator.device)
        ...
        if infer_config is None:
            ...
            infer_config_dict = OmegaConf.load(infer_config)
        ...
        if infer_config is not None:
            ...
            print('number of inference samples:', len(infer_config_dict))

            for i in range(len(infer_config_dict)):

                seed_num = infer_config_dict[i].get('seed_num', 1)


                for s in range(seed_num):
                    print(f"Validation on prompt: {s+1}/{seed_num}")
                    ...
                    if seed_num == 1:
                        ...
                        if s == 0:
                            ...
                            generator = torch.Generator(device=accelerator.device).manual_seed(random_seed)
                    ...
                    with torch.no_grad():
                        if args.train_mode != "normal":
                            with torch.autocast("cuda", dtype=weight_dtype):
                                ...
                                input_video, input_video_mask, _ = get_image_to_video_latent(infer_config_dict[i].get("image_path") , None, video_length=video_length, sample_size=[h, w])


                                sample = pipeline(
                                    infer_config_dict[i].get("text"),
                                ...
                                save_videos_grid(sample, os.path.join(args.output_dir, f"sample/sample-{i}-{random_seed}-g{args.guidance_scale}.mp4"))
        else:
                ...
                with torch.no_grad():
                    if args.train_mode != "normal":
                        with torch.autocast("cuda", dtype=weight_dtype):
                            ...
                            os.makedirs(os.path.join(args.output_dir, "sample"), exist_ok=True)
                            save_videos_grid(sample, os.path.join(args.output_dir, f"sample/sample-{global_step}-{i}-g{args.guidance_scale}.mp4"))
...
def parse_args():
    ...
    parser.add_argument(
        "--lora_path", type=str, default=None, help="If you want to load the LoRA weight, input its path." 
    )

    parser.add_argument(
        "--infer_config", type=str, default=None, help="The config for inference."
    )

    parser.add_argument(
        "--guidance_scale", type=float, default=6.0, help="The guidance_scale for inference."
    )
