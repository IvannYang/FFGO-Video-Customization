...
class ImageVideoDataset(Dataset):
    def __init__(
        ...
        self.still_image_rate = still_image_rate
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image')=='video':
            ...
            with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
                ...
                # print("video length, clip_length, start_idx, batch_index", video_length, clip_length)
                # '''
                ...
                try:
                    ...
                    for i in range(len(pixel_values)):
                        ...
                        '''
                        frame shape, resized_frame shape (640, 360, 3) (640, 384, 3)
                        '''
                    ...
                    if np.random.rand() < self.still_image_rate:
                        # 1. random repeat a frame
                        # repeat_index = np.random.randint(0, len(resized_frames))
                        # resized_frames= [resized_frames[repeat_index]] * len(resized_frames)

                        # 2. permute with repeat
                        indices = np.random.choice(len(resized_frames), size=len(resized_frames), replace=True)
                        ...
                        # print("pixel_values after using still image", pixel_values.shape)
                        # print('text segments', text_segments)
                        # print('new text', text)
                        # exit(0)
                    # else:
                    #     pixel_values= np.array(resized_frames)
                    #     print('use video', pixel_values.shape)
                    #     exit(0)
