...
class Wan2_2FunInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # print('latents', latents.shape, latents.dtype, latents.device)
        # print("mask_latents shape", None if mask_latents is None else mask_latents.shape)
        # print("masked_video_latents shape", None if masked_video_latents is None else masked_video_latents.shape)
        # '''
        # latents torch.Size([1, 48, 21, 20, 12]) torch.bfloat16 cuda:0
        # mask_latents shape torch.Size([1, 4, 21, 20, 12])
        # masked_video_latents shape torch.Size([1, 48, 21, 20, 12])
        # '''
        # exit(0)
