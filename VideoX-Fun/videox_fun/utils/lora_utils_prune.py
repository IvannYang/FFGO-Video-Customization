            ...
            for name, module in root_module.named_modules():
                if module.__class__.__name__ in target_replace_modules:
                    ...
                    for child_name, child_module in module.named_modules():
                        ...
                        # print(f"-> sub module {child_name} ({child_module.__class__.__name__})")

                        # print("first 10 named modules:")
                        # for i, (n, m) in enumerate(module.named_modules()):
                        #     print(f"  {n.replace(".", "_")} ({m.__class__.__name__})")
                        #     if i >= 50:
                        #         break

                        # print("saved layers list:", saved_layers_list)
                        # exit(0)

                        if saved_layers_list is not None and len(saved_layers_list) > 0:
                            if child_name.replace(".", "_") not in saved_layers_list:
                                continue 
...
def create_network(
    ...
    **kwargs,
    ...
    network = LoRANetwork(
        ...
        skip_name=skip_name,
        varbose=True,
        saved_layers_list=saved_layers_list
    )
    return network



def merge_lora_ours(pipeline, lora_path, multiplier, device='cpu', dtype=torch.float32, state_dict=None, transformer_only=False, sub_transformer_name="transformer", lora_threshold= -1):
    LORA_PREFIX_TRANSFORMER = "lora_unet"
    LORA_PREFIX_TEXT_ENCODER = "lora_te"
    ...
    for layer, elems in updates.items():
        ...
        if len(weight_up.shape) == 4:
            ...
            if mag_dict['rel_fro'] > lora_threshold:
